...
def process_file_parallel(input_file, limit, num_ranks, output_name, chunk_size=10000):
    ...
    try:
        ...
        columns = min(6, len(lines[0].strip().split('|')))
        ...
        # Create output files
        output_files = [open(f"{output_name}{col}.txt", 'w') for col in range(columns)]
...
def main():
    ...
    limit = 750000
    num_ranks = 4
    output_name = "/p/pd/pim/newarchitecture/test_with_communication_to_cpu/8channel/morebankchange/oldrankopt/lineorder_LatencyRankoptd_ch1_sf1_C"    
    ...
    limit = 7500000
    num_ranks = 4
    output_name = "/p/pd/pim/newarchitecture/test_with_communication_to_cpu/8channel/morebankchange/oldrankopt/lineorder_LatencyRankoptd_ch1_sf10_C"    
    print(f"Starting to process file: {input_file}")
    print(f"Processing up to {limit} lines with {num_ranks} ranks")
    print(f"Using {mp.cpu_count()} CPU cores")
    ...
    limit = 75000000
    num_ranks = 4
    output_name = "/p/pd/pim/newarchitecture/test_with_communication_to_cpu/8channel/morebankchange/oldrankopt/lineorder_LatencyRankoptd_ch1_sf100_C"    
    print(f"Starting to process file: {input_file}")
    print(f"Processing up to {limit} lines with {num_ranks} ranks")
    print(f"Using {mp.cpu_count()} CPU cores")
