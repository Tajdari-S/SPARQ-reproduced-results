...
def initialize_dask():
    ...
    cluster = LocalCUDACluster(
        rmm_pool_size='36GiB',  # Adjust based on GPU capacity
        protocol="ucx",
    ...
    client.run(rmm.reinitialize,
               pool_allocator=True,
               initial_pool_size='36GiB')  # Match pool size with worker memory
...
def process_join(left_df, right_df, left_key, right_key):
    """Perform join and get metrics without materializing full dataset."""
    try:
        start = time.time()
        ...
        # Compute metadata without materializing
        joined = joined.persist()
        count = joined.shape[0].compute()
        duration = time.time() - start
        
        print(f"Join completed: {count:,} rows in {duration:.2f}s")
        del joined  # Explicit cleanup
        return duration
    except Exception as e:
