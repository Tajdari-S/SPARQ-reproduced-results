#!/usr/bin/env bash
# Figure 11's 13 SSB queries at SF100 under DuckDB v0.8.0, with or without
# binding DuckDB's memory to one socket (that socket's 8 DDR4 channels).
# CPU threads are never restricted.
#
#   MODE=none bash fig11_membind.sh     # as the paper measured: no binding
#   MODE=mem1 bash fig11_membind.sh     # numactl --membind=1 only
#
# Loads the five tables in memory once, exactly as DucDBSSBFullQUery.sh does, then
# runs every query NUM_RUNS times in the same process with JSON profiling on and
# records the profile's latency. Run 1 of Q1.1 is the only run that follows the
# load directly; the paper's script started one process per query instead, so
# only warm runs (2..NUM_RUNS) compare to its cold/warm split.
# Needs ~110 GB of memory on the bound node; our peak was 107 GB.
set -uo pipefail

DUCKDB="${DUCKDB:-/p/pd/newgem5/gem5/duckdb}"   # v0.8.0, the binary Figure 11 used
TAG="${TAG:-rep1}"
...
OUT="${OUTDIR}/fig11_${MODE}_${TAG}.csv"
PROFDIR="${WORK:-/tmp/fig11_membind}/prof_${MODE}_${TAG}"
...
echo "binary: ${DUCKDB}   mode: ${MODE} (${PIN:-no binding})   profiles -> ${PROFDIR}"
echo "measurement: PRAGMA enable_profiling='json' + profiling_output, as in DucDBSSBFullQUery.sh"
ALLRUNS=""
while IFS='|' read -r qn qs; do
  [ -z "$qn" ] && continue
  for i in $(seq 1 "$NUM_RUNS"); do
    ALLRUNS="${ALLRUNS}PRAGMA profiling_output = '${PROFDIR}/Query_${qn}_run$(printf %02d $i).json'; ${qs}
"
  done
done < <(queries)
echo "--- one process: load once, then all queries $(date '+%H:%M:%S') ---"
/usr/bin/time -v $PIN "$DUCKDB" >/dev/null 2>"${OUTDIR}/time_${MODE}_${TAG}.txt" <<SQL
${SETUP}
PRAGMA enable_profiling='json';
${ALLRUNS}
SQL
RC=$?
echo "  rc=$RC  peak RSS $(grep -o 'Maximum resident set size (kbytes): [0-9]*' "${OUTDIR}/time_${MODE}_${TAG}.txt" | awk '{printf "%.1f GB", $NF/1048576}')  done $(date '+%H:%M:%S')"
python3 "$(dirname "${BASH_SOURCE[0]}")/extract_profiles.py" "$PROFDIR" "$RC" >> "$OUT"
