...
def initialize_dask():
    ...
    cluster = LocalCUDACluster(
        rmm_pool_size='20GiB',  # Adjust based on GPU capacity
        protocol="ucx",
    ...
    client.run(rmm.reinitialize,
               pool_allocator=True,
               initial_pool_size='20GiB')  # Match pool size with worker memory
    ...
    try:
        if file_path == '/p/pd/ssb-dbgen/sf100/':
            df = dask_cudf.read_csv(
                ...
                sep=',',
                names=column_names,
                blocksize="256 MiB",  # Control partition size
            ...
            df = dask_cudf.read_csv(
                ...
                sep='|',
                names=column_names,
                blocksize="1024 MiB",  # Control partition size
                dtype='int32',  # Auto-detect numeric columns
            )
            print(f"Lazily loaded {file_path} | Partitions: {df.npartitions}")
            return df
...
def optimize_schema(df, numeric_cols):
    ...
    return df  # No persist() to avoid OOM

def process_join(left_df, right_df, left_key, right_key):
    """Perform join and measure pure computation latency with precise timing."""
    try:
        # Ensure GPU is idle and all prior operations are complete
        cp.cuda.Device(0).synchronize()
        start_setup = cp.cuda.Event()  # For pre-join setup
        start_join = cp.cuda.Event()   # For join execution
        end_join = cp.cuda.Event()     # End of join
        
        # Record setup time (e.g., graph construction)
        start_setup.record()
        ...
        # Record right before join computation starts
        cp.cuda.Device(0).synchronize()  # Ensure graph is ready
        start_join.record()
        
        # Compute only the row count to minimize memory usage
        joined_result = joined.persist()  # Compute join asynchronously
        dask.compute(joined_result)  # Wait for join to fully compute
        
        # Record right after join completion
        end_join.record()
        end_join.synchronize()  # Ensure timing is complete
        
        # Calculate timings
        setup_duration = cp.cuda.get_elapsed_time(start_setup, start_join)  # s
        join_duration = cp.cuda.get_elapsed_time(start_join, end_join)    # s
        joined_size = joined.shape[0].compute()
        print(f"Setup time (pre-join): {setup_duration:.4f} s")
        print(f"Join completed: {joined_size:,} rows in {join_duration:.4f} s (pure join time)")
...
def main():
    ...
    dask.config.set({
        "distributed.worker.memory.target": 0.5,  # More aggressive memory management
