...
def process_join(left_df, right_df, left_key, right_key, transmit_to_cpu=False, client=None):
    ...
    try:
        ...
        start_setup = time.time()  # For pre-join setup
        ...
        compute_duration = (end_compute - start_compute)   # s
        setup_duration = (start_compute - start_setup)   # s
        
        if transmit_to_cpu:
            start_transfer = time.time()
            result = joined.compute()  # Brings result to CPU
            end_transfer = time.time()
            transfer_duration = (end_transfer - start_transfer)  # s
            print(f"Join computed in {compute_duration:.4f}s (GPU time), "
                  f"transferred to CPU in {transfer_duration:.4f}s, "
                  f"result has {len(result):,} rows")
            print(f"Join setup in {setup_duration:.4f}s (GPU time), "
                  f"result has {len(result):,} rows")
            del result
        else:
        ...
        del joined
        return compute_duration, setup_duration, transfer_duration if transmit_to_cpu else None
    except Exception as e:
...
def main():
    ...
    scale_factors = [
        ...
        ('100', '/p/pd/ssb-dbgen/sf100/', False, "256 MiB")
    ]
    ...
    for scale, path, transmit_to_cpu, blocksize in scale_factors:
        ...
        for left_tbl, right_tbl, lkey, rkey in joins:
            ...
            compute_duration, setup_duration, transfer_duration = process_join(
                tables[left_tbl],
                tables[right_tbl],
                lkey,
                rkey,
