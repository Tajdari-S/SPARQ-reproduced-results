...
def process_chunk(chunk_data):
    ...
    for i, line in enumerate(lines):
        ...
        for col, base_binary in enumerate(binaries):
            ...
            column = int(binary[-12:-4], 2)
            bank = int(binary[-2:], 2)
            bank_group = int(binary[-4:-2], 2)
...
def main():
    ...
    limit = 750000
    num_ranks = 4
    output_name = "/p/pd/pim/newarchitecture/test_with_communication_to_cpu/8channel/morebankchange/lineorder_BWopt_ch1_sf1_C"    
    ...
    limit = 7500000
    num_ranks = 4
    output_name = "/p/pd/pim/newarchitecture/test_with_communication_to_cpu/8channel/morebankchange/lineorder_BWopt_ch1_sf10_C"    
    print(f"Starting to process file: {input_file}")
    print(f"Processing up to {limit} lines with {num_ranks} ranks")
    print(f"Using {mp.cpu_count()} CPU cores")
    ...
    limit = 75000000
    num_ranks = 4
    output_name = "/p/pd/pim/newarchitecture/test_with_communication_to_cpu/8channel/morebankchange/lineorder_BWopt_ch1_sf100_C"    
    print(f"Starting to process file: {input_file}")
    print(f"Processing up to {limit} lines with {num_ranks} ranks")
    print(f"Using {mp.cpu_count()} CPU cores")
